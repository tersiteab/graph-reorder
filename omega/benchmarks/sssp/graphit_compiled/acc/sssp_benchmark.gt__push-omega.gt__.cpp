...
template <typename APPLY_FUNC > VertexSubset<NodeID>* edgeset_apply_push_parallel_weighted_deduplicatied_from_vertexset_with_frontier_allInSPM(WGraph & g , VertexSubset<NodeID>* from_vertexset, APPLY_FUNC apply_func) 
{ 
    int64_t numVertices = g.num_nodes(), numEdges = g.num_edges();
    from_vertexset->toSparse();
    long m = from_vertexset->size();
...
bool operator() (NodeID src, NodeID dst, int weight) 
  {
    ...
    if (omegaInSPM(dst)) {
      ...
      SP_trackving_var_1 = (1);
    } else {
      SP_trackving_var_1 = writeMin( &SP[dst], ((volatile int*)SP)[src] + weight ); 
    }
...
int main(int argc, char * argv[])
{
  ...
  for ( int trail = (0) ; trail < (1) ; trail++ )
  {
    ...
    while ( (builtin_getVertexSetSize(frontier) ) != ((0) ))
    {
      ...
      if (omegaAllInSPM()) {
        frontier = edgeset_apply_push_parallel_weighted_deduplicatied_from_vertexset_with_frontier_allInSPM(edges, frontier, updateEdge_allInSPM()); 
      } else {
