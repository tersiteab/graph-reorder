...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename APPLY_FUNC , typename PUSH_APPLY_FUNC> VertexSubset<NodeID>* edgeset_apply_hybrid_dense_parallel_weighted_deduplicatied_from_vertexset_with_frontier_allInSPM(WGraph & g , VertexSubset<NodeID>* from_vertexset, APPLY_FUNC apply_func, PUSH_APPLY_FUNC push_apply_func) 
{ 
    int64_t numVertices = g.num_nodes(), numEdges = g.num_edges();
    from_vertexset->toSparse();
    long m = from_vertexset->size();
...
bool operator() (NodeID src, NodeID dst, int weight) 
  {
    ...
    if (omegaInSPM(dst)) {
      omegaWriteX(((volatile int*)SP)[src] + weight, dst);
      SP_trackving_var_3 = (1);
    } else {
      SP_trackving_var_3 = writeMin( &SP[dst], (((volatile int*)SP)[src] + weight) ); 
    }
...
int main(int argc, char * argv[])
{
  ...
  for ( int trail = (0) ; trail < (1) ; trail++ )
  {
    ...
    while ( (builtin_getVertexSetSize(frontier) ) != ((0) ))
    {
      ...
      if (omegaAllInSPM()) {
        frontier = edgeset_apply_hybrid_dense_parallel_weighted_deduplicatied_from_vertexset_with_frontier_allInSPM(edges, frontier, updateEdge(), updateEdge_push_ver_allInSPM()); 
      } else {
