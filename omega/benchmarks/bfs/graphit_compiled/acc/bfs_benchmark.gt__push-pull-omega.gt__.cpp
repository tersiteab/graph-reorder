  ...
  return next_frontier;
    ...
    uintT offset = offsets[i];
        for(NodeID d : g.out_neigh(s)){
          if (to_func(d)) {                                   // Shouldn't this not be here?
...
template <typename TO_FUNC , typename APPLY_FUNC, typename PUSH_APPLY_FUNC> VertexSubset<NodeID>* edgeset_apply_hybrid_dense_parallel_from_vertexset_to_filter_func_with_frontier_allInSPM(Graph & g , VertexSubset<NodeID>* from_vertexset, TO_FUNC to_func, APPLY_FUNC apply_func, PUSH_APPLY_FUNC push_apply_func) 
{ 
    int64_t numVertices = g.num_nodes(), numEdges = g.num_edges();
    from_vertexset->toSparse();
    long m = from_vertexset->size();
...
int main(int argc, char * argv[])
{
  ...
  for ( int trail = (0) ; trail < (1) ; trail++ )
  {
    ...
    {
      if (omegaAllInSPM()) {
        frontier = edgeset_apply_hybrid_dense_parallel_from_vertexset_to_filter_func_with_frontier_allInSPM(edges, frontier, toFilter(), updateEdge(), updateEdge_push_ver_allInSPM()); 
      } else {
